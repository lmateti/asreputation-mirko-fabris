...
##############################################################################
open(INPUT, 'rib.20110113.0000') or die "Could not open INPUT $!\n";

open(MYOUTFILE, ">rib.20110113.0000.parsed");
...
sub decode_mrt_packet {
	...
	if ($type == MSG_TABLE_DUMP) { ###########################################
		...
		if ($format == 1) {
			...
			print MYOUTFILE "PREFIX: " . inet_ntop($af, $prefix) . "/$prefixlen\n";
			print 'FROM: ' . inet_ntop($af, $peerip) . " AS$peer_as\n"
				if $peer_as;
			print MYOUTFILE 'FROM: ' . inet_ntop($af, $peerip) . "\n";
			print MYOUTFILE 'AS_PATH:'  . print_aspath($attr->[BGP_ATTR_AS_PATH]) . "\n";
		...
		while (length $rest > 0) {
			...
			if ($format == 1) {
				...
				print 'FROM: ' . inet_ntop($af, $BGP_Peers[$peer_index]->[2])
					. " AS$peer_as\n";
				print MYOUTFILE 'FROM: ' . inet_ntop($af, $BGP_Peers[$peer_index]->[2]) . "\n";
				print MYOUTFILE 'AS_PATH:'  . print_aspath($attr->[BGP_ATTR_AS_PATH]) . "\n"
				if $attr->[BGP_ATTR_AS_PATH];
